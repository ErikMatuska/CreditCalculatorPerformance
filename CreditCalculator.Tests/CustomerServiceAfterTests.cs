...
public class CustomerServiceAfterTests
{
        ...
        string email)
    {
        ...
        companyRepository.Setup(cr => cr.GetById(It.IsAny<int>())).Returns(new Company { Id = 1, Type = CompanyType.RegularClient });

        var creditLimitCalculator = new Mock<ICalculator>();
        creditLimitCalculator.Setup(clc => clc.Calculate(It.IsAny<Company>(), It.IsAny<Customer>())).Returns((false, 0));

        var dateTimeProvider = new Mock<IDateTimeProvider>();
        dateTimeProvider.Setup(dtp => dtp.Now).Returns(new DateTime(2024,1,1));
